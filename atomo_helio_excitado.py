...
r_B, e_B = None, None
V_total_B = None
V_iniciales = [[0, 0.967, 0.973, 0.93, 0.975],[0.249, 0.152, 0.152, 0.159, 0.165]]
...
for i in range(5):
    if i == 0:     
        ...
        energyloc_He_B1 = energies_location(initial_conditions, u, d2r_helium_atom_initial, e_v, constants_He)
        for energy_pair in energyloc_He_B1:
            r_B, e_B = energy_search(initial_conditions, u, d2r_helium_atom_initial, constants_He, energy_pair, 0.001)
        ...
        plt.figure(figsize=(12,8))
        #Para A
        e_v = np.linspace(-0.85, -0.75, 1000)
        if i >= 2:
            e_v = np.linspace(-1.0, -0.8, 100)
        ...
        for energy_pair in energyloc_He_B[1:2]:
